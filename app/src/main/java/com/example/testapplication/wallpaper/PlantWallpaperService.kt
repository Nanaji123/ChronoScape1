...
class PlantWallpaperService : WallpaperService() {
        ...
        inner class PlantEngine : Engine() {
                ...
                private val grassSeeds =
                        (0..140).map { Pair(Random.nextFloat(), Random.nextFloat()) }
                private val weedSeeds = (0..70).map { Pair(Random.nextFloat(), Random.nextFloat()) }
                private val bigPlantSeeds =
                        (0..15).map { Pair(Random.nextFloat(), Random.nextFloat()) }
                ...
                private val rainDropSeeds =
                        (0..80).map { Pair(Random.nextFloat(), Random.nextFloat()) }
                ...
                private val drawRunnable =
                        object : Runnable {
                                override fun run() {
                                        ...
                                        val durationMinutes = sessionManager.getGrowthDuration()

                                        // Target g is 2000. Loop delay is 33ms.
                                        // increment = targetG / totalSteps
                                        // totalSteps = (durationMinutes * 60 * 1000) / 33
                                        val increment =
                                                if (durationMinutes > 0f) {
                                                        (2000f * 33f) / (durationMinutes * 60000f)
                                                } else {
                                                        0.5f // Default slow growth if duration is
                                                        // invalid
                                                }

                                        if (g < 2000f) sessionManager.saveGrowth(g + increment)
                                        draw()
                ...
                private fun draw() {
                        ...
                        try {
                                ...
                                // Season: every 300 growth = one season cycle
                ...
                private fun drawStars(canvas: Canvas, w: Float, h: Float) {
                        repeat(starSeeds.size) { i ->
                                ...
                                val twinkle =
                                        (sin(windTime.toDouble() * 1.8 + i).toFloat() + 1f) / 2f
                ...
                private fun drawSunMoon(
                        ...
                        if (dayPhase >= 0f) {
                                ...
                                repeat(8) { i ->
                                        val ang =
                                                (i.toFloat() / 8f) * Math.PI.toFloat() * 2f +
                                                        windTime * 0.25f
                                        val inner = 30f
                                        val outer =
                                                42f +
                                                        sin(windTime.toDouble() * 2.2 + i)
                                                                .toFloat() * 5f
                                        canvas.drawLine(
                                                objX + cos(ang) * inner,
                                                objY + sin(ang) * inner,
                                ...
                                // Crescent: full disc then overlay with background-coloured circle
                                // offset
                ...
                private fun drawClouds(
                        ...
                        repeat(3) { ci ->
                                ...
                                val rawX =
                                        w + 180f -
                                                ((cloudTime * 45f + cloudOffsets[ci] * (w + 360f)) %
                                                        (w + 360f))
                                val cloudX = rawX
                ...
                private fun drawGround(
                        ...
                        val groundColor1 =
                                if (s1 == 3) Color.parseColor("#2A2A3A")
                                else Color.parseColor("#1A0E07")
                        val groundColor2 =
                                if (s2 == 3) Color.parseColor("#2A2A3A")
                                else Color.parseColor("#1A0E07")
                ...
                private fun drawGroundVegetation(
                        ...
                        repeat(bigPlantSeeds.size) { i ->
                                ...
                                val curH = bH * appear
                                val w1 =
                                        sin(windTime.toDouble() * 0.8 + x / 50.0).toFloat() *
                                                appear *
                                                10f
                                val w2 =
                                        sin(windTime.toDouble() * 1.3 + x / 30.0 + i).toFloat() *
                                                appear *
                                                5f

                                weedPaint.color =
                                        if (i % 2 == 0) Color.parseColor("#1B5E20")
                        ...
                        repeat(weedSeeds.size) { i ->
                                ...
                                val w1 =
                                        sin(windTime.toDouble() * 1.0 + x / 38.0).toFloat() *
                                                appear *
                                                9f
                                val w2 =
                                        sin(windTime.toDouble() * 1.4 + x / 22.0 + i * 0.5)
                                                .toFloat() * appear * 4f
                                val tipX = x + w1 + w2
                                ...
                                val w3 =
                                        sin(windTime.toDouble() * 1.1 + x / 28.0 + 1.2).toFloat() *
                                                appear *
                                                7f
                                val b1 = Path()
                                b1.moveTo(x, groundY)
                                ...
                                val w4 =
                                        sin(windTime.toDouble() * 0.9 + x / 32.0 + 2.4).toFloat() *
                                                appear *
                                                7f
                                val b2 = Path()
                                b2.moveTo(x, groundY)
                        ...
                        repeat(grassSeeds.size) { i ->
                                ...
                                val curH = maxH * appear
                                val w1 =
                                        sin(windTime.toDouble() * 1.2 + x / 25.0).toFloat() *
                                                appear *
                                                10f
                                val w2 =
                                        sin(windTime.toDouble() * 0.8 + x / 18.0 + i).toFloat() *
                                                appear *
                                                4f

                                grassPaint.color =
                                        if (i % 2 == 0) Color.parseColor("#43A047")
                                        else Color.parseColor("#2E7D32")
                        ...
                        repeat(flowerSeeds.size) { i ->
                                ...
                                val w1 =
                                        sin(windTime.toDouble() * 1.0 + x / 42.0).toFloat() *
                                                appear *
                                                8f
                                val w2 =
                                        sin(windTime.toDouble() * 1.5 + x / 20.0 + i).toFloat() *
                                                appear *
                                                3f
                                val tipX = x + w1 + w2
                                ...
                                val col =
                                        Color.parseColor(
                                                flowerColors[
                                                        (i + (sz * 10).toInt()) % flowerColors.size]
                                        )
                ...
                private fun drawTree(
                        ...
                        // DENSE BRANCHING: Start at 0.18f, and many points towards the top to fill
                        // space.
                        listOf(0.18f, 0.30f, 0.42f, 0.54f, 0.66f, 0.78f, 0.88f, 0.93f, 0.96f, 0.99f)
                                .forEachIndexed { idx, frac ->
                                        ...
                                        val bLen =
                                                trunkH * (0.42f - idx * 0.02f).coerceAtLeast(0.15f)
                        ...
                        if (crownAlpha > 0) {
                                ...
                                repeat(6) { i ->
                                        ...
                                        val bx =
                                                crownX +
                                                        cos(angleOffset + windTime * 0.3)
                                                                .toFloat() * dist
                                        val by =
                                                crownY +
                                                        sin(angleOffset + windTime * 0.3)
                                                                .toFloat() * dist - i * 10f
                                        drawLeafCluster(
                                                canvas,
                                                bx,
                ...
                private fun drawBR(
                        ...
                        val dGrowth =
                                ((growth - (250f + spawnIdx * 200f + depth * 220f) - growthOffset) /
                                                140f)
                                        .coerceIn(0f, 1f)
                        ...
                        val swayAmp =
                                windSway * ((depth + 1).toFloat() / (maxDepth + 1).toFloat()) * 2.0f
                        ...
                        val p =
                                if (depth <= 2)
                                        branchPaint.apply {
                                                strokeWidth = thickness.coerceAtLeast(3f)
                                        }
                ...
                private fun drawLeafCluster(
                        ...
                        repeat(innerCount) { i ->
                                val ang =
                                        (i.toFloat() / innerCount) * Math.PI.toFloat() * 2f +
                                                sway / 12f +
                                                0.4f
                ...
                private fun drawGroundVines(
                        ...
                        repeat(5) { vi ->
                                ...
                                for (s in 0..50) {
                                        ...
                                        val und =
                                                sin(t * Math.PI.toFloat() * 4f + seed * 6f)
                                                        .toFloat() * 5f
                                        val wo =
                                                sin(windTime.toDouble() + t * 8.0 + vi).toFloat() *
                                                        3f *
                                                        reachFrac
